...
function colorGrid (selector, options) {
  ...
  forIn(options.gridItems, function(k, v) {
    ...
    if (typeof v === 'string') {
      itemValue.temp[V] = { hex: itemValue[k] };
      itemValue.temp[V].rgb = hexToRGB( itemValue.temp[V].hex );
      itemValue.temp[V].rgba = rgbToRgba( itemValue.temp[V].rgb );
    }
    V++;
  });
  ...
  if (selectorsLength < gridItemsObjectLength) {
    ...
    forEach(reminingItems, function (i, e) {
      if (i < gridItemsObjectLength - 1)
  ...
  var t = 0, tl = Object.keys(options.gridItems).length - 1;
  forIn(colorHolderList, function(i,e) {
    if ( t < tl ) {
      grid_container.insertAdjacentHTML('beforeend', gridItemHTML);
      gridItem = doc.getElementsByClassName('colorsGrid-item')[t];

      gridItem.setAttribute('data-hex-value', options.gridItems.temp[t].hex);
      gridItem.setAttribute('data-rgb-value', options.gridItems.temp[t].rgb);
      gridItem.setAttribute('data-rgba-value', options.gridItems.temp[t].rgba);
      ...
      colorHolderList[t].style.backgroundColor = options.gridItems.temp[t].hex;

      gridItem.appendChild(colorHolderList[t]);
